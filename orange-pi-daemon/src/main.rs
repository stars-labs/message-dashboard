...
async fn main() -> Result<()> {
    ...
    if marked_deleted > 0 {
        info!("✅ Marked {} old uploaded messages as deleted (stale paths)", marked_deleted);
    }
    ...
    let worker_pool = Arc::new(WorkerPool::new(worker_config.clone(), modem_manager.clone()));
    info!("👷 Worker pool initialized with {} parallel workers (batch size: {})", worker_config.num_workers, worker_config.batch_size);
    ...
    info!("🚀 Starting DUAL LOOP architecture with {} modems", valid_modems.len());
    ...
    tokio::spawn(async move {
        loop {
            ...
            match modem_reader_pool.process_modems(modem_reader_ids.clone()).await {
                Ok(results) => {
                    let mut count = 0;
                    ...
                    for result in results {
                        for msg_with_path in result.messages_with_paths {
                            ...
                            match modem_reader_store.store_message(&msg_with_path.message, &msg_with_path.modem_id, &msg_with_path.sms_path) {
                                Ok(true) => {
                                    count += 1;

                                    // DELETE from SIM immediately (ALL messages - FIX for duplication bug)
                                    match modem_reader_manager.delete_sms(&msg_with_path.modem_id, &msg_with_path.sms_path).await {
                                        Ok(_) => {
                                            deleted_count += 1;
                                            debug!("✅ Deleted SMS from SIM: {} (timestamp: {})",
                                                   &msg_with_path.sms_path, &msg_with_path.message.timestamp);
                                        }
                                        Err(e) => {
    ...
    tokio::spawn(async move {
        info!("☁️  Dynamic batch uploader started - SMART SIZING!");
        ...
        loop {
            ...
            match upload_store.get_pending_messages(current_batch_size) {
                Ok(pending) if !pending.is_empty() => {
                    ...
                    let payload_size: usize = messages.iter()
                    ...
                    if payload_size < 100_000 && ids.len() == current_batch_size {
                        // Small payload, can increase batch size
                        current_batch_size = (current_batch_size * 2).min(100);
                    } else if payload_size > 500_000 {
                        // Large payload, decrease batch size
                        current_batch_size = (current_batch_size / 2).max(10);
                    }

                    info!("☁️  Uploading {} messages (~{} KB)", ids.len(), payload_size / 1024);
                    ...
                    match upload_client.upload_messages(&messages).await {
                        Ok(_) => {
                            ...
                            // Small delay to avoid rate limits (2x speed vs original)
                        ...
                        Err(e) => {
                            ...
                            // Only delay on failure to avoid hammering a broken API
                            tokio::time::sleep(Duration::from_secs(1)).await;
                        }
                    }
                }
                _ => {
                    // No pending messages, minimal yield to prevent CPU spinning
                    // This is only 1ms instead of 100ms!
                    tokio::time::sleep(Duration::from_millis(1)).await;
                }
            }
            // NO DELAY between loops - continuous processing at maximum speed!
        }
    });
    ...
    let sync_manager_arc = Arc::new(tokio::sync::Mutex::new(SyncManager::new(session_id.clone(), 300)));
        ...
        loop {
            ...
            match sync_pool.process_modems(sync_ids.clone()).await {
                Ok(results) => {
                    ...
                    let modems: Vec<Modem> = all_phones.iter().map(|p| Modem {
                    ...
                    match sync_client.upload_devices(&modems, &all_sims, sync_mode, &session_id).await {
                        Ok(_) => {
                            info!("📊 Status sync: Updated {} devices (mode: {:?})",
                                  all_phones.len(), sync_mode);
    ...
    tokio::spawn(async move {
        loop {
            ...
            if let Ok(stats) = stats_store.get_stats() {
                info!("📊 Database stats: {} pending, {} uploading, {} uploaded, {} failed",
