...
{#if phone}
  <div class="tech-card p-4 {mobile ? 'mx-4 mb-4' : 'mb-4'}">
      ...
      <div>
        ...
        <p class="text-sm text-gray-400 mt-1">
          ...
          {#if phone.operator_name && !phone.mapped_carrier}
            {#if phone.carrier} • {/if}
            <span class="text-gray-400">{phone.operator_name}</span>
          {/if}
        </p>
      </div>
    ...
    <div class="mt-4 space-y-2 text-sm bg-gray-800/50 rounded-lg p-3 border border-gray-700">
      {#if phone.iccid}
        <div class="flex justify-between">
          <span class="text-gray-400/70">ICCID:</span>
          <span class="font-mono text-xs text-gray-200">{phone.iccid}</span>
        </div>
      {/if}
      {#if phone.modem_index != null || phone.sim_index != null}
        <div class="flex justify-between">
          <span class="text-gray-400/70">位置索引:</span>
      ...
      {#if phone.mapped_number}
        <div class="flex justify-between items-center">
          <span class="text-gray-400/70">映射来源:</span>
      ...
      {#if phone.imei}
        <div class="flex justify-between">
          <span class="text-gray-400/70">IMEI:</span>
          <span class="font-mono text-xs text-gray-200">{phone.imei}</span>
        </div>
      {/if}
      {#if phone.mapped_carrier || phone.operator_name}
        <div class="flex justify-between items-center">
          <span class="text-gray-400/70">运营商:</span>
          <span>
            ...
            {:else}
              <span class="text-gray-200">{phone.operator_name} {phone.operator_id ? `(${phone.operator_id})` : ''}</span>
            {/if}
          </span>
        </div>
      {/if}
      {#if phone.access_tech}
        <div class="flex justify-between">
          <span class="text-gray-400/70">网络类型:</span>
          <span class="uppercase font-medium text-gray-200">{phone.access_tech}</span>
        </div>
      {/if}
      {#if phone.country}
        <div class="flex justify-between">
          <span class="text-gray-400/70">国家/地区:</span>
          <span class="text-gray-200">{phone.country}</span>
        </div>
      {/if}
      {#if phone.lastActive}
        <div class="flex justify-between">
          <span class="text-gray-400/70">最后活跃:</span>
          <span class="text-gray-200">{new Date(phone.lastActive).toLocaleString('zh-CN')}</span>
        </div>
      {/if}
    </div>
  </div>
