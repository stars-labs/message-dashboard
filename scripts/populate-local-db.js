#!/usr/bin/env node
// Script to populate local D1 sims table directly from CSV
import fs from 'fs';
...
async function main() {
  ...
  const sqlFile = '/tmp/populate-sims.sql';
  fs.writeFileSync(sqlFile, sql);

  console.log('Executing SQL via wrangler...');

  try {
    const { stdout, stderr } = await execAsync(`cd sms-dashboard && bunx wrangler d1 execute sms-dashboard --local --file=${sqlFile}`);
    console.log(stdout);
    if (stderr) console.error(stderr);
    console.log(`\n✓ Successfully inserted ${records.length} SIM records`);
  } catch (error) {
  ...
  // Verify
  try {
    const { stdout } = await execAsync(`cd sms-dashboard && bunx wrangler d1 execute sms-dashboard --local --command="SELECT COUNT(*) as count FROM sims"`);
    console.log('\nVerification:');
    console.log(stdout);
  } catch (error) {
